...
def _api_request(
    ...
    if resp.status_code >= 400 or (isinstance(data, dict) and data.get("ok") is False):
        code = (data.get("code") if isinstance(data, dict) else None) or f"HTTP_{resp.status_code}"
        err = (data.get("error") if isinstance(data, dict) else None) or data.get("message") if isinstance(
            data, dict
        ) else "Request failed"
...
def call_invite_api(team_id: str, auth: str, member_email: str):
    """
    Gửi lời mời member qua endpoint v2:
...
def call_team_invites_api(team_id: str, auth: str):
    """
    GET /api/v2/teams/:id/invites
    """
    return _api_request("GET", f"/api/v2/teams/{team_id}/invites", auth_token=auth, timeout=30)
...
def invite_with_failover(auth: str, member_email: str, max_size: int):
    teams_payload = call_teams_api(auth=auth)
    # Một số implementation trả về {"data": [...], "total": ...}
    # một số khác trả về {"data": {"teams": [...]}}. Hỗ trợ cả hai.
    raw_data = teams_payload.get("data") or {}
    if isinstance(raw_data, dict):
        teams_data = raw_data.get("teams") or []
    else:
        teams_data = raw_data or []
    team_ids: list[str] = []
